...
if (System::typeOf($sets['types'], 'scalar')) {
	$sets['types'] = [ $sets['types'] ];
}

foreach ($sets['types'] as $item) {
	
	$url = 'https://api.openweathermap.org/data/2.5/' . $item . '?' . (System::type($sets['weather'], 'numeric') ? 'id=' : 'q=') . $sets['weather'] . '&units=metric&lang=ru&appid=' . $sets['key'];
	
	$folder = $this -> cache;
	//$file = $folder . datadatetime('', $sets['savestate']) . '.' . $item . '.ini';
	$file = $folder . date('YmdH') . '.' . $item . '.ini';
	
	Local::createFolder($folder);
	
	if (file_exists($file)) {
		$temp = Local::readFile($file);
	} else {
		$temp = Local::requestUrl($url, null, 'post');
		//$temp = local::openUrl($url);
		if (!empty($temp)) {
			Local::eraseFolder($folder);
			Local::writeFile($file, $temp);
		}
	}
	unset($file, $folder);
...
$data = [
	'number' => date('w', $jsonstr['weather']['dt']) + 1,
...
$data['description'] = Strings::join($data['description'], ', ');
unset($item);
if (System::set($data['code'])) {
...
foreach ($jsonstr['forecast'] as $item) {
	...
	$date = date('Ymd', $item['dt']);
	$curr = date('Ymd');
	$number = date('w', $item['dt']);
	$time = (int) date('H', $item['dt']);
	...
	if ($time < 6) {
		$type = 'night';
		//$date = date('Ymd', $item['dt'] - TIME_DAY / 2);
		$date = date('Ymd', $item['dt'] - 86400 / 2);
	} elseif ($time < 12) {
		$type = 'morning';
	} elseif ($time > 18) {
		$type = 'night';
...
foreach (['morning', 'day', 'night'] as $key) {
	foreach ($data['daily'] as &$item) {
		...
		$item[$key]['humidity'] = System::set($item[$key]['humidity']) ? round(array_sum($item[$key]['humidity']) / count($item[$key]['humidity'])) : null;
		$item[$key]['pressure'] = System::set($item[$key]['pressure']) ? round(array_sum($item[$key]['pressure']) / count($item[$key]['pressure'])) : null;
		$item[$key]['wind'] = System::set($item[$key]['wind']) ? round(array_sum($item[$key]['wind']) / count($item[$key]['wind'])) : null;
		$item[$key]['direction'] = System::set($item[$key]['direction']) ? round(array_sum($item[$key]['direction']) / count($item[$key]['direction'])) : null;
